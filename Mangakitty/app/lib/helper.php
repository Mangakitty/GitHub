<?php
	...
	// This is for page's params only ((routing)), use it to get parameters from router, never use set_parrams, use G() instead
	// If you declare a variable in a template files (views), this will not work in shortcode because shortcode come first then views after, 
	// try TE() instead, this will get $template variable
	function P($key, $defaul = ""){
		return WASD::pparams($key, $default);
	}

	// This is global value, 2nd parameter is value (if you want to set, not default value as usual, remember)
	function G($key, $value = ""){
		return WASD::gglobal($key, $value);
	}

	function TE($key, $default = ''){
		return isset(Template::$vars[$key]) ? Template::$vars[$key] : $default;
	}
